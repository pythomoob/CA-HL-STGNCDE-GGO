...
def GGO(func, Solution, nb_solu, taille_solu, itération):
    ...
    vecteur_Fonc_objectiv_meillure = np.full(shape=(T + 1), fill_value=Matrice_Solution[idxMin, taille_solution],
                                             dtype=float)
    _, UB, _, LB = func(([1, 2]))
    ...
    while t <= T:
        ...
        for i in range(n1):
            if (t % 2) == 0:
                if r3 < 0.5:
                    if abs(A) < 1:
                        Matrice_Solution[i, :-1] = solution_best_in_itra[:-1] - A * abs(
                            C * solution_best_in_itra[:-1] - Matrice_Solution[i, :-1])
                    else:
                        ...
                        while (Sa == idxMin) or (Sb == idxMin) or (Sc == idxMin):
                            ...
                            while Sb == Sc:
                                Sb = random.randint(0, n1 - 1)
                        Matrice_Solution[i, :-1] = w1 * Matrice_Solution[Sa, :-1] + z * w2 * (
                                    Matrice_Solution[Sb, :-1] - Matrice_Solution[Sc, :-1]) + (1 - z) * w3 * (
                                                               Matrice_Solution[i, :-1] - Matrice_Solution[Sa, :-1])
                else:
                    Matrice_Solution[i, :-1] = w4 * abs(
                        solution_best_in_itra[:-1] - Matrice_Solution[i, :-1]) * math.exp(b * l) * math.cos(
                        2 * math.pi * l) + (2 * w1 * (r4 + r5)) * solution_best_in_itra[:-1]
            else:
                Matrice_Solution[i, :-1] = Matrice_Solution[i, :-1] + (1 + z) * w * (
                            Matrice_Solution[i, :-1] - solution_best_in_itra[:-1])
        ...
        for i in range(n2):
            if (t % 2) == 0:
                ...
                S1 = Matrice_Solution[n1 + x1, :-1] - A * abs(
                    C * Matrice_Solution[n1 + x1, :-1] - Matrice_Solution[n1 + i, :-1])
                S2 = Matrice_Solution[n1 + x2, :-1] - A * abs(
                    C * Matrice_Solution[n1 + x2, :-1] - Matrice_Solution[n1 + i, :-1])
                S3 = Matrice_Solution[n1 + x3, :-1] - A * abs(
                    C * Matrice_Solution[n1 + x3, :-1] - Matrice_Solution[n1 + i, :-1])
                Matrice_Solution[n1 + i, :-1] = (S1 + S2 + S3) / 3
            else:
                Matrice_Solution[n1 + i, :-1] = Matrice_Solution[n1 + i, :-1] + (1 + z) * w * (
                            Matrice_Solution[n1 + i, :-1] - solution_best_in_itra[:-1])
            for j in range(taille_solution):
        ...
        if (vecteur_Fonc_objectiv_meillure[t] >= vecteur_Fonc_objectiv_meillure[t - 1] or
            vecteur_Fonc_objectiv_meillure[t] >= vecteur_Fonc_objectiv_meillure[t - 2]) and n1 < nb_solution - 1:
            n1 = n1 + 1
            n2 = n2 - 1
        else:
            n2 = n2 + 1
